...
public class ProvinciaServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        List<VolProvincia> list = ubigeoDao.obtenerProvincias(codDepartamento);
        try {
            for (VolProvincia provincia : list) {
                out.printf("<option value='%1s'>%2s</option>", provincia.getCodProvincia(), provincia.getNombrepro());
            }
        } finally {
            out.close();
        }
