...
public class DistritoServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        List<VolDistrito> list = ubigeoDao.obtenerDistritos(codProvincia);
        try {
            for (VolDistrito distrito : list) {
                out.printf("<option value='%1s'>%2s</option>", distrito.getCodDistrito(), distrito.getNombredis());
            }
        } finally {
            out.close();
        }
